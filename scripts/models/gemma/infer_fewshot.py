...
# -*- coding: utf-8 -*-
"""
Gemma 3 27B few-shot inference for seven tasks for benchmarking iconicity.
Uses 3 examples from different iconicity categories (arbitrary, high iconic, low iconic).
"""
...
# Old build_iconicity_prompt function removed - now using proper conversation structure
...
def build_messages_for_video_with_fewshot(video_path: str, task: str, test_gloss: str, fewshot_examples: List[Dict], video_dir: Path):
    """Build proper few-shot conversation with clear example-answer pairs."""
    messages = [
        {
            "role": "system",
    ...
    for i, example in enumerate(fewshot_examples):
        ...
        # Extract frames for this example
        frames = extract_video_frames(fewshot_path, max_frames=8)
        if not frames:
            print(f"Warning: Could not extract frames from {fewshot_path}")
            continue
        ...
        # User message with video frames and question
        user_content = []
        for frame in frames:
    ...
    test_frames = extract_video_frames(video_path, max_frames=8)
    if not test_frames:
        print(f"Warning: Could not extract frames from {video_path}")
        return messages
    
    # Build question for test case
    test_question = build_task_question(task, test_gloss)
    
    # Final user message with test video and question
    test_content = []
    for frame in test_frames:
        test_content.append({"type": "image", "image": frame})
    test_content.append({"type": "text", "text": test_question})
...
def parse_args():
    p = argparse.ArgumentParser(description="Gemma 3 27B Iconicity few-shot inference")
