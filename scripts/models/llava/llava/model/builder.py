...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if load_8bit:
        # Use BitsAndBytesConfig API for 8-bit quantization (avoid deprecated load_in_8bit kwarg)
        kwargs["quantization_config"] = BitsAndBytesConfig(load_in_8bit=True)
    elif load_4bit:
        # Use BitsAndBytesConfig API for 4-bit and avoid passing load_in_4bit kwarg simultaneously
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                # Always load the LlavaQwenConfig to ensure correct model_type and fields
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    from llava.model.language_model.llava_qwen import LlavaQwenConfig
                    # Always construct a proper LlavaQwenConfig for Qwen-based LLaVA models
                    llava_cfg = LlavaQwenConfig.from_pretrained(model_path)
                    if overwrite_config is not None:
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if not vision_tower.is_loaded:
            # Force the vision tower to a single GPU to avoid sharding mismatches
            vt_device_map = "auto" if device_map == "auto" else {"": 0}
            vision_tower.load_model(device_map=vt_device_map)
        # Ensure the vision tower is on a real CUDA device with the model's dtype (avoid meta tensors)
        target_dtype = getattr(model, 'dtype', torch.float16)
        if target_dtype not in (torch.float16, torch.bfloat16):
            target_dtype = torch.float16
        # If accelerate handles placement (auto), do not override with .to();
        # only move explicitly when a fixed device_map is used.
        if device_map != "auto":
            try:
