...
# -*- coding: utf-8 -*-
"""
Qwen2.5-VL zero-shot inference for seven tasks for benchmarking iconicity."""
        ...
        for it in tqdm(items, desc="Processing all tasks"):
            ...
            for task in TASKS:
                ...
                try:
                    ...
                    text = instruction
                if task == "transparency_2":
                    print(f"{vid} | transparency_2 prompt: {text}")
                inputs = processor(text=[text], images=image_inputs, videos=video_inputs, padding=True, return_tensors="pt")
                inputs = inputs.to(model.device)

                # Generate with standardized parameters
...
def parse_args():
    p = argparse.ArgumentParser(description="Qwen2.5-VL Iconicity inference (local)")
